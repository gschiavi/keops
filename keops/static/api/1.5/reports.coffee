...
class Report
  ...
  saveDialog: ->
    ...
    name = window.prompt(Katrid.i18n.gettext('Report name'), @userReport.name)
    if name
      @userReport.name = name
      $.ajax
        type: 'POST'
  ...
  loadUserReport: (id) ->
    @action.location.search('user_report', id)
  ...
  export: (format='pdf') ->
    ...
    svc.post 'export_report', null, { args: [@info.report_id], kwargs: { format: format, params: params } }
    .done (res) ->
      if res.result.open
...
class Param
  constructor: (@info, @params) ->
    ...
    console.log('param info', @params.info)
    dt = dtypeDict[@type.toLowerCase()]
    if dt
      @type = dt
    if @info.sql_choices
      @type = 'sqlchoices'
